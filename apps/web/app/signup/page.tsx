      ...
      <main className="relative z-10 flex-1 flex flex-col lg:flex-row items-center justify-center w-full max-w-7xl mx-auto px-6 lg:px-12 py-12 lg:py-0">
        <div className="w-full lg:w-1/2 flex flex-col justify-center items-start lg:pr-16 mb-12 lg:mb-0 pt-16 lg:pt-0">
            <h1 className="font-extrabold tracking-tight text-zinc-900 mb-6 flex flex-col">
              <span className="text-6xl sm:text-7xl lg:text-8xl leading-[0.9] text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 pb-2">Start</span> 
              <span className="text-3xl sm:text-4xl lg:text-5xl font-semibold text-zinc-800 tracking-tight mt-[-4px]">journey on the canvas.</span>
            </h1>
        ...
        <div className="w-full lg:w-1/2 flex justify-center lg:justify-end">
          <div className="w-full max-w-[420px] relative overflow-hidden rounded-2xl border border-zinc-200 bg-white/80 backdrop-blur-xl shadow-[0_8px_40px_rgb(0,0,0,0.08)] p-8 sm:p-10">
            <div className="mb-8">
              <h2 className="text-xl font-semibold tracking-tight text-zinc-900 mb-2">Create account</h2>
              <p className="text-sm text-zinc-500">Enter your details below to create your workspace.</p>
            </div>

            <form onSubmit={handleEmailSignUp} className="space-y-5" noValidate>
              <div className="space-y-1.5">
                <label htmlFor="name" className="block text-xs font-medium text-zinc-500 tracking-wide uppercase">Full Name</label>
                <input id="name" name="name" type="text" autoComplete="name" value={name} onChange={(e) => setName(e.target.value)} required placeholder="John Doe" className="w-full h-11 px-3 text-[15px] text-zinc-900 placeholder-zinc-400 bg-zinc-50 border border-zinc-200 rounded-lg focus:outline-none focus:border-zinc-400 focus:ring-1 focus:ring-zinc-200 transition-all" disabled={isLoading} />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <input id="password" name="password" type="password" autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} required placeholder="Create a password" className="w-full h-11 px-3 text-[15px] text-zinc-900 placeholder-zinc-400 bg-zinc-50 border border-zinc-200 rounded-lg focus:outline-none focus:border-zinc-400 focus:ring-1 focus:ring-zinc-200 transition-all" disabled={isLoading} />
              </div>
              <button type="submit" disabled={isLoading} className="w-full h-11 mt-2 flex items-center justify-center rounded-lg bg-zinc-900 text-white text-[14px] font-semibold tracking-wide hover:bg-zinc-800 focus:outline-none focus:ring-2 focus:ring-zinc-400 focus:ring-offset-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
                {isLoading ? <div className="h-4 w-4 animate-spin rounded-full border-2 border-zinc-500 border-t-white" /> : 'Create Account'}
              </button>
            </form>
            ...
            <div className="grid grid-cols-2 gap-3">
                <button onClick={() => window.location.href = `${API_URL}/api/v1/auth/google`} className="flex items-center justify-center gap-2 h-10 rounded-lg border border-zinc-200 bg-white hover:bg-zinc-50 text-zinc-700 text-sm font-medium transition-colors shadow-sm">
                ...
                <button onClick={() => window.location.href = `${API_URL}/api/v1/auth/github`} className="flex items-center justify-center gap-2 h-10 rounded-lg border border-zinc-200 bg-white hover:bg-zinc-50 text-zinc-700 text-sm font-medium transition-colors shadow-sm">
            ...
            <p className="mt-8 text-center text-sm text-zinc-500">
              ...
              <Link href="/signin" className="font-medium text-zinc-900 hover:underline underline-offset-4">Sign in</Link>
            </p>
          </div>
        </div>
      </main>
